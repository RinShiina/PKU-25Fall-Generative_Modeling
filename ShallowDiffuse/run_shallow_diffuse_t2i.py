...
async def diff_watermark(idx, edit_timestep, seed, output_folder_timestep, gt_patch, current_prompt_embeddings, null_embeddings, current_prompt, ref_model, ref_clip_preprocess, ref_tokenizer, device, pipe, args):
    ...
    xt_no_w_latent = pipe.backward_diffusion(
        ...
        start_timestep=0,
        end_timestep=args.num_inference_steps - edit_timestep,
    )
   ...
   # inject watermark
    ...
    # ==========================================
    # 可视化代码：复现 Figure 3 和 Figure 1
    # ==========================================

    # 1. 保存 Mask (Figure 3 左上：白色圆环掩码)
    # 取第0个样本，指定的通道，转成 numpy
    mask_visual = watermarking_mask[0, args.w_channel].float().cpu().numpy()
    plt.imsave(f'{output_dir_path}/vis_mask.png', mask_visual, cmap='gray')

    # 2. 保存 Pattern (Figure 3 左下：水印噪声图案)
    # gt_patch 是频域数据，我们取它的幅度(abs)或者实部(real)来看
    # 注意：gt_patch 的形状可能和 mask 不一样，如果是全图噪声，形状是一样的
    if gt_patch.shape[-1] == mask_visual.shape[-1]:
        pattern_visual = gt_patch[0, args.w_channel].abs().cpu().numpy()
        # 用 log1p 压缩一下动态范围，不然看不清
        pattern_visual = np.log1p(pattern_visual)
        plt.imsave(f'{output_dir_path}/vis_pattern.png', pattern_visual, cmap='viridis')

    # 3. 保存 Latent 的频域图 (Figure 3 右侧：频域分布)
    # 对无水印的 Latent 做 FFT
    fft_no_w = torch.fft.fft2(xt_no_w_latent)
    # fftshift 把低频移到中心
    fft_visual = torch.fft.fftshift(fft_no_w)[0, args.w_channel].abs().cpu().numpy()
    fft_visual = np.log1p(fft_visual) # log 压缩方便观察
    plt.imsave(f'{output_dir_path}/vis_latent_freq.png', fft_visual, cmap='inferno')
    # ==========================================

    # === 注入水印 ===
    xt_w_latent = inject_watermark(xt_w_latent, watermarking_mask, gt_patch, w_injection=args.w_injection)
    ############################ step 4: xt->x0, xt'->x0', nocfg, cond
    ...
    with open(f'{output_dir_path}/distance.txt', 'a') as f:
        f.write('='*15)
        f.write('clip similarity score\n')
        f.write(f'w_no_sim = {w_no_sim}\n')
        f.write(f'avg_sim = {avg_sim}\n')
    final_result['clip_scores_no_w'] = w_no_sim
    final_result['clip_scores_avg'] = avg_sim
...
async def main(args, edit_t, pipe, scheduler, dataset, prompt_key, ref_model, ref_clip_preprocess, ref_tokenizer):
    ...
    for i in tqdm(range(args.start, args.end)):
        ...
        diff_watermark_task = asyncio.create_task(diff_watermark(
            ...
            gt_patch = gt_patch,
            current_prompt_embeddings = current_prompt_embeddings,
    ...
    with open(f'{output_folder_timestep}/clip_scores.txt', 'a') as f:
        f.write('CLIP scores\n')
        f.write(f"clip_score_mean: {mean(final_result_list['clip_scores_no_w'])}\n")
        f.write(f"avg_clip_score_mean: {mean(final_result_list['clip_scores_avg'])}\n")
